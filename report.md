...
|---|---|---|---|---|
| 1 | **표지 — "AI가 SAST를 대체할 수 있는가. 아직, 아니다."** | 0:20 | 결론을 먼저 던지되, 근거는 지금부터 보여준다고 예고 | 실행 콘솔 터미널 (6건 → CONFIRMED 1건, 환각 3건) |
| 2 | **아젠다** | 0:20 | "실패부터 판다. 그다음 설계." 발표 원칙을 명시 | 챕터별 시간 배분표 |
| 3 | **CH01 · 5대 한계** | 1:00 | 진짜 질문은 "코드 넣고 물으면 되나"가 아니라 "이 5개 한계를 아키텍처로 얼마나 상쇄하는가" | AI SAST 5대 한계 카드 (#2 강조) |
| 4 | **CH02 · Target userland** | 1:00 | 아카이브된 레거시 C 118,474줄. 스코프 10종 중 6종이 2025 Top 25 안에 있다 | pygount 실측 + MITRE 순위·KEV 표 |
| 5 | **CH03 · 아키텍처** | 1:10 | **"LLM은 샌드위치의 속이지 빵이 아니다"** — 이 발표의 한 줄 | 4단계 파이프라인 (결정론/LLM 이중 언어) |
| 6 | **CH04 · Chunking + 토큰 절감** | 1:00 | 배치 우선순위는 코드 양이 아니라 **신뢰 경계** | Tier 규칙 + 50.7% 실측 + 5계층 |
| 7 | **CH05 · 3개 배치 결과** | 1:00 | 자동 파이프라인이 낸 6건 → **최종 CONFIRMED 1건**. "CONFIRMED만 3건"을 의도적으로 피했다 | 배치별 판정 카드 3장 |
| 8 | **CH06 · ⚡ 환각 3건** | **1:40** | **클라이맥스.** 구문적 환각 2건은 코드가 잡았지만, "함수·라인 다 맞는데 결론이 틀린" 의미적 환각은 Grounding Check도 통과시켰고 사람이 원본 재대조로만 잡아냈다 | 환각 3유형 코드 대조 (모노스페이스) |
| 9 | **CH07 · 프롬프트 엔지니어링** | 1:10 | "잘 검증해줘"는 무시된다. 그래서 생각을 **필수 필드로 강제**했다 | (B) 개발 중 실제 입력 프롬프트 + (A) 도구 내부 프롬프트 |
| 10 | **CH08 · v2 → v3 Evidence Contract** | 1:00 | *"검증하라"는 지시로는 검증을 강제할 수 없다. 진짜 대조는 코드로 한다* | v2 체크 3종 + v3 스키마 |
| 11 | **CH09 · 3자의 상호 사각지대** | 1:00 | 셋은 서로 다른 곳을 본다. 이 표본에서 자동 파이프라인이 먼저 찾은 최종 정탐은 없다 | §9 3행 비교표 |
| 12 | **CH10 · 결론 + 5대 한계 대응** | 1:00 | **"이번 실험에서 AI가 더 많은 최종 정탐을 확보했다는 증거는 없다"** — 가치는 대체가 아니라 분석·triage·검증 보조 | 5대 한계 대응 매핑표 (해결 수준 점 스케일) |

**합계 약 10분 20초** — 표지·아젠다(합 40초)를 빠르게 넘기면 10분에 맞는다. 슬라이드 8에 가장 많은 시간을 배정한 이유는, 이 프로젝트의 진짜 기여가 "취약점을 찾았다"가 아니라 **"검증 계층 자체의 한계를 실측하고 다음 설계로 연결했다"**는 데 있기 때문이다.
