...
| 분할한 코드 배치 중 **3개 배치에 대한 결과** | **B083 / B097 / B011** — **서로 다른 공격 표면을 대표하도록 선정**(Device Tree 파서 / MP4 컨테이너 파서 / 동시성 프리미티브). 실행 결과 서로 다른 판정 유형이 관측되었다. B011은 최초 CONFIRMED가 사람 재검증으로 REJECTED 정정된 과정까지 기록(§8.3) | §8, `agent_results/*.json` |
...
| 작성 시 요청했던 **프롬프트** | §17.1 실사용 프롬프트 **전문** + §7.3 v0→v1→v2 버전 히스토리 |
...
| ④ 설득력 있는 발표 구성 | §16 **12장·10분** 발표 시나리오 (실제 제작된 `SAST_Agent_발표.pptx`와 1:1 일치) — 실패 사례(그중 가장 위험한 의미적 환각)를 클라이맥스에 배치 |
...
**"AI SAST는 기존 SAST의 대체재가 아니라, 룰 엔진의 결과를 설명·검토하고 미탐 가능 영역에서 추가 후보를 생성하는 보완재다."**

> **표현을 정밀하게 쓰는 이유**: "룰 엔진이 놓치는 구간을 메운다"고 쓰면 *AI가 룰 엔진의 미탐을 실제로 메웠다*는 주장이 된다. **이번 3개 배치에서 그 증거는 얻지 못했다**(§9.2, §12.2). 최종 CONFIRMED 1건은 Cppcheck도 이미 찾은 것이다. 확인된 것은 (a) 룰 엔진 진단의 도달 조건·맥락 설명, (b) 룰 엔진이 보지 않은 영역에서의 **추가 검토 후보 생성**, (c) 불확실성의 구조화다. 이 세 가지가 이 보고서가 방어할 수 있는 보완 가치의 범위다.

본 보고서는 이 명제를 주장으로 끝내지 않고, 실제로 구현·실행하여 **실측 데이터로 증명**한다. 특히:

- Rule Engine(Cppcheck)이 **0건**을 낸 파일에서 LLM이 검토 후보를 생성한 사례 → 최소 설정 룰 엔진 단독의 한계 실증(§8.1)
- LLM이 **존재하지 않는 코드를 취약점으로 지어낸 사례 2건**과, 그것을 자동으로 걸러낸 방어 장치, 그리고 **그 방어 장치조차 통과한 의미적 환각 1건** → 환각 실증 및 대응의 한계(§10)
- 사람이 직접 추적해서 찾은 취약점과 자동 파이프라인 결과의 **부분 일치 + 상호 사각지대** → 완전 자동화의 한계 실증(§9)
...
대상인 `userland`는 Raspberry Pi GPU 인터페이스용 **저수준 C 라이브러리**다. **Top 25를 그대로 적용하면 이 코드베이스와 관련성이 낮은 웹 중심 CWE에 분석 예산이 상당 부분 소모될 수 있다.** XSS/SQLi/CSRF를 찾으라고 시키는 것은 토큰 낭비일 뿐 아니라, 모델이 "뭐라도 찾아야 한다"는 압력을 받아 오탐을 만들어내는 직접적 원인이 된다.

따라서 **저수준 C 코드에 특화된 CWE 10종(메모리 안전성·정수·동시성·명령 실행)으로 스코프를 재조준했다**:
...
- **CWE-416(Use After Free)은 KEV 등재 CVE 14건**으로 Top 25 전체에서 두 번째로 실전 악용이 많다. 본 파이프라인은 B011에서 이 CWE-416 후보를 생성했으나, **사람 재검증에서 의미적 환각으로 판명되어 기각됐다**(§8.3, §10.3). 스코프에 이 CWE를 포함한 판단 자체는 타당했지만, 이번 실험에서 확정된 CWE-416 취약점은 없다.
...
실측 토큰 비율(§11.1)을 적용하면 **C 소스 전체 1회 투입 시 약 111만 토큰**이 필요하다. Analyzer + Verifier 2-pass 구조에서는 약 222만 토큰이다(정확한 산출식은 §11.2). 이는:
...
flowchart TD
    ...
    I["Phase 6: Dedup & Ranking<br/>(Planned · 미구현)"] --> J
...
파이프라인은 Phase 0~7, 총 8개 Phase로 구성되며 그중 LLM을 쓰는 건 2개 Phase(Analyzer, Verifier)뿐이다. 이건 비용 문제만이 아니다.
...
이 둘은 **목적함수가 정반대**라 하나의 프롬프트로 합칠 수 없다. 반면 "Dedup Agent", "Ranking Agent"를 LLM으로 만드는 건 낭비다. 정렬과 중복 제거는 **결정론적 코드로 구현하는 것이 적합하다**.

> **구현 상태 명시**: 다만 Phase 6(Dedup & Ranking)은 **현재 설계 단계이며 코드로 구현되어 있지 않다.** 3개 배치만 실행해 중복 finding이 발생하지 않았기 때문에 구현 필요가 없었다(§14.8). 아키텍처 다이어그램에도 `(Planned)`로 표시했다. "LLM으로 만들지 않는다"는 판단과 "이미 만들었다"는 사실은 별개이므로 구분해서 적는다.
...
| **구현** | Cppcheck CLI를 `subprocess`로 호출, **stderr에 출력되는 XML 결과를 파싱 없이 원문 그대로** Analyzer 프롬프트에 전달 |
| **입력** | 배치의 `.c` 파일 목록 |
...
| 근거 | 설명 |
|---|---|
| **컨텍스트 정확도** | 컨텍스트 크기·반복 오버헤드·재시도 비용을 고려한 **운영상 휴리스틱**으로 설정. 본 프로젝트는 chunk-size별 비교 실험(2,000 / 2,500 / 3,000줄 ablation)을 수행하지 않았으므로, 이 값이 최적이라는 실측 근거는 없다 |
...
과제 필수조건은 "3개 배치 결과"다. 아무거나 3개가 아니라, **서로 다른 공격 표면을 대표하도록** 선정했다.

> **선정 시점을 분명히 한다**: 세 배치는 **파이프라인을 실행하기 전에**, Tier 규칙과 공격 표면 특성만 보고 골랐다. 판정 결과를 보고 고른 것이 아니다(그 시점엔 결과가 존재하지 않았다). 아래 "예상 검증 목표"는 선정 근거이고, 실제 판정은 §8의 실행 결과다. **결과적으로 서로 다른 판정 유형이 관측된 것은 선정 기준의 부산물이지 선정 사유가 아니다.**

| Batch | 디렉터리 | Tier | 줄 수 | 파일 | 검증 초점 |
|---|---|---|---|---|---|
| **B083** | `helpers/dtoverlay` | 1 | 2,992 | `dtoverlay.c` | Cppcheck 진단과의 교차검증 (Cppcheck 히트 있음) |
| **B097** | `containers/mp4` | 1 | 1,879 | `mp4_reader.c` | 외부 파일 포맷 파서 검증 (Cppcheck 0건) |
| **B011** | `interface/vcos/generic` | 2 | 2,257 | `vcos_cmd.c`, `vcos_logcat.c`, `vcos_generic_blockpool.c`, `vcos_msgqueue.c` | 동시성 프리미티브 검토 (한계 #2 정면 시험) |

서로 다른 공격 표면을 대표하는 3개 배치를 **실행 전에** 선정했다. 결과적으로 CONFIRMED·REJECTED·정정 사례가 함께 관측됐다 — 이는 선정 시점에 알 수 없었던 실행 결과이지, 선정 기준이 아니다.
...
> **Cppcheck을 이 목록에서 제외한 이유**: 초고에서는 "Rule Engine 선행 — Cppcheck 0토큰"을 절감 항목으로 넣었으나, 이는 **부정확한 프레이밍이다.** Cppcheck 자체는 API를 호출하지 않으니 그 실행에 토큰이 들지 않는 것은 맞지만, **그 출력(XML)은 고스란히 Analyzer 프롬프트에 텍스트로 추가된다** — 즉 Gemini에 들어가는 입력 토큰을 오히려 소폭 늘린다. LLM 호출 자체를 없애거나 코드 입력량을 줄이는 게 아니므로 "절감 장치"가 아니다. Cppcheck의 정확한 역할은 **"0-토큰 결정론적 품질 보조/사전 힌트"**이며, 이는 아키텍처의 신뢰도 계층(§3, §4.5)에 속하는 것이지 토큰 절감 계층이 아니다.

### 6.1 계층별 요약

| # | 방안 | 계층 | 도입 이유 | 실측/근거 |
|---|---|---|---|---|
| 1 | **C 분석 대상 외 파일 배제** | Explorer | 스코프가 C 메모리 안전성이므로 그 외 파일은 입력에서 제외 | 828개 중 **183개(22.1%) 제외** — C++·Objective-C·CMake·Makefile·Groff 등 |
| 2 | **Tier 기반 조기 종료** | Chunker | 115개 배치 전부 돌릴 필요 없음. 공격 표면 상위부터 | Tier 1~2만 실행 시 전체 대비 대폭 절감 |
| 3 | **출력 스키마·Evidence 길이 제한** | Prompt schema | `response_mime_type=json` 강제 + evidence 필드를 "최대 2문장, 원문 코드 인용 5줄 이내"로 제한(§17.1 스키마)해 불필요한 산문·장문 생성을 억제 | Analyzer 프롬프트에 실제 명시된 제약(코드 원문 대조 확인) |
| 4 | **Verifier 조건부 호출** | Orchestration | Analyzer 후보 0건이면 검증할 대상이 없음 | **B011 실측: 42,875 → 21,127 토큰 (50.7% 절감)** |
| 5 | **Thinking 최소화 + JSON 강제** | API 파라미터 | 내부 추론 토큰이 출력 예산을 잠식 | 초기 실행에서 비가시 토큰 **3,928개 추정** → 조치 후 **Run C에서 inferred 값 0 관측**(§8.5) |
...
→ **50.7% 절감**. 동일 배치·동일 코드에서 관측된 값이다.

> **이 수치의 한계를 명확히 한다**: 두 관측은 **모델이 서로 다르다**(Run A는 flash, Run B는 flash-lite). 따라서 이것은 "Verifier 호출을 켜고 끈 통제 실험(ablation)"이 아니라, **동일 배치에서 Verifier가 호출된 경우와 호출되지 않은 경우의 토큰량을 대조한 관측 사례**다. 조건부 호출이 Verifier 1회분(약 21K 토큰)을 그대로 절약한다는 산술은 자명하지만, 엄밀한 절감률 주장을 하려면 **동일 모델·동일 입력에서 Verifier 강제 호출 ON/OFF 실험**이 별도로 필요하다. 본 보고서는 그 실험을 수행하지 않았다.
...
### 7.2 구현 당시 AI에게 실제로 보낸 프롬프트 (원문)

> 산출물 요구사항 *"작성 시 요청했던 프롬프트"* 는 두 가지로 읽힐 수 있다. **(A) 도구 내부에 하드코딩된 Analyzer/Verifier 프롬프트**(§17.1 전문)와, **(B) 이 도구를 만드는 과정에서 개발자가 AI 어시스턴트에게 실제로 입력한 프롬프트**다. 아래는 (B)이며, 실제 작업 세션 로그에서 원문 그대로 옮긴 것이다.

#### 프롬프트 1 — 과제 정의 단계 (최초 입력)

```
자 우리가 이제 SAST를 만드는 과제가 생겼어

* AI SAST의 주요 한계점 → 과제와 연관
   * Context Window & 대용량 소스 제약
      * 너무 큰 코드, 토큰이 많이 쓰임
   * 환각에 의한 오탐/미탐
      * 존재하지 않는 취약점 생성(오탐)
      * 비동기 레이스 컨디션 미탐 위험
   ...
   * 동적 런타임 환경 미반영
   * 사내 자체 프레임워크 학습 부재
* 과제 산출물 꼭 md파일로 내기, PDF 금지!

이 내용도 필기에 적혀있고 내가 첨부한 사진들을 보고
내가 정확히 구체적으로 어떤 걸 어떤 기준에 맞춰 해야하는지 정리해줘
```

**의도**: 강의 필기와 과제 슬라이드 사진을 함께 주고, **AI가 요구사항을 먼저 구조화하게 만들었다.** 내가 이해한 것과 AI가 읽어낸 것을 대조해 누락을 잡기 위함이다.

#### 프롬프트 2 — 설계 교차검증 (다른 AI의 답변을 가져와 대조)

```
너가 말한 거 + 다른 애가 말한 거를 가져왔는데
이 둘을 적절히 조합해서 정확하게 과제에 필요한 내용들을 반영해서
내가 어떤 순서로 어떻게 해야하는지 자세한 파이프라인을 짜줘
```

**의도**: **단일 AI의 답을 신뢰하지 않고, 서로 다른 모델의 설계안을 교차 검증**했다. 이 프로젝트의 핵심 철학(§10.6 "AI 출력은 코드로 검증한다")이 개발 프로세스 자체에도 적용된 지점이다. 이 프롬프트의 결과로 Cppcheck 선행 + 2-Agent 구조가 확정됐다.

#### 프롬프트 3 — 검증 요구 (반복 사용한 지시)

```
무조건 있는 출처로만 근거있는 출처로만 자료조사 후 작업 수행,
교차 검증 수행, 법령 등은 항상 날짜 확인 후 최신으로 작업하기
```

**의도**: AI의 환각을 개발 단계에서부터 차단. 이 한 줄 때문에 **Cppcheck 저장소 이관(`danmar` → `cppcheck-opensource`), CWE Top 25 실제 순위, Gemini 무료 티어 쿼터** 등이 1차 출처로 재확인됐고, 그 과정에서 초고의 오류 6건이 정정됐다(§17.4 정정 이력).

#### 프롬프트 4 — 사람이 먼저 손으로 분석하기 위한 요청

```
Get-CodeContext "helpers\dtoverlay\dtoverlay.c" 2320 15
Select-String -Path "containers\mp4\mp4_reader.c" -Pattern "malloc|memcpy|strcpy|read\(|alloc\("
```

**의도**: 프롬프트를 쓰기 전에 **사람이 직접 코드를 추적**했다(§7.1 단계 1). 이 수동 분석 과정에서 드러난 판단 순서가 그대로 Analyzer/Verifier 프롬프트의 체크리스트가 됐다.

#### 프롬프트 5 — 실패 관측 후 아키텍처 변경 요청

실행 결과에서 환각이 CONFIRMED로 통과한 것을 확인한 뒤:

```
> python agent_pipeline.py B083 B097 B011
=== B083 ===
  -> 3건 검증 완료 (CONFIRMED 3건)
...
음 근데 에이전트 결과 파일은 바뀐 게 있는 게 맞나? 없는 게 맞나? 흠
```

**의도**: **결과를 그대로 수용하지 않고 의심**했다. 이 의심에서 B083의 환각 2건이 발각됐고, "프롬프트를 더 세게 쓰자"가 아니라 **"결정론적 `grounding_check()`를 추가하자"**는 아키텍처 결정으로 이어졌다(§7.3 v1→v2).

#### 프롬프트 6 — 리뷰 반영 요청

```
이런 피드백이 왔어 참고해서 반영할 부분만 반영해서 수정할 수 있어?
그리고 왜 반영했는지 왜 반영하지 않았는지 근거까지 다 말해주고
```

**의도**: **피드백을 무비판적으로 수용하지 않기 위해** "반영할 부분만"과 "반영하지 않은 근거"를 함께 요구했다. 실제로 이 지시 때문에, 리뷰어가 "§8.4를 재구성 예정으로 낮추라"고 한 지적이 **사실과 반대**(이미 push 완료)임을 확인하고 §14.7 쪽을 정정하는 반대 조치를 취할 수 있었다.

#### 이 6개 프롬프트에서 반복되는 패턴

| 패턴 | 해당 프롬프트 | 효과 |
|---|---|---|
| **AI에게 먼저 구조화를 시킨다** | 1 | 요구사항 누락 방지 |
| **복수 AI의 답을 교차 검증한다** | 2 | 단일 모델 편향 제거 |
| **출처·검증을 상시 지시로 건다** | 3 | 개발 단계 환각 차단 → 오류 6건 정정 |
| **프롬프트 전에 사람이 먼저 해본다** | 4 | 사람의 판단 순서를 프롬프트로 번역 |
| **결과를 의심한다** | 5 | 환각 3건 발각 → 아키텍처 변경 |
| **피드백도 검증 대상으로 둔다** | 6 | 잘못된 지적 1건 역정정 |

> **핵심**: 이 프로젝트에서 AI를 쓴 방식은 "시키고 받기"가 아니라 **"시키고, 의심하고, 대조하고, 정정하기"**였다. 도구 안에 넣은 Verifier·Grounding Check 구조가 사실은 **개발 과정에서 사람이 AI를 대하던 방식을 그대로 코드로 옮긴 것**이다.
...
- CWE 화이트리스트 10종(CWE-787/120/125/416/415/476/190/362/78/401) + 웹 취약점 명시적 금지
...
**동일 코드, 동일 프롬프트, 동일 파라미터에서 후보 생성 결과가 3건 → 0건으로 바뀌었다.**

> **recall 근거로 쓸 수 있는 것과 없는 것을 구분한다.** B011에서 사라진 1건(CWE-416)은 **애초에 오탐이었으므로**(§8.3) 그것이 사라진 것은 recall 저하의 근거가 되지 못한다 — 오히려 결과적으로는 옳은 방향이다. **recall 저하의 유효한 근거는 B083이다**: 사람이 검증해 CONFIRMED로 확정한 CWE-476(line 2320)을 flash-lite가 Run B와 Run C에서 **두 번 모두 놓쳤다.** 확정된 실제 취약점을 반복적으로 놓쳤다는 것이 유일하게 방어 가능한 recall 주장이다.
...
> **정정(§14.7 반영 완료)**: 초고 시점에는 저장소에 Run B 결과만 남아있었으나, 리뷰 이후 `agent_results/`를 `run_A_gemini_flash/` · `run_B_flash_lite/` · `manual_ground_truth/`로 재구성해 세 실행 결과를 모두 저장소에 보존했다. Run A의 실제 finding 값은 본 보고서 §8.2~8.3에 전문 기록되어 있으며, Run A의 토큰 실측치는 §11에 정리했다.

### 8.5 Run C — 패치 이후 검증 실행 (§14.6 신뢰성 패치 확인용)

`agent_pipeline.py`에 §14.5~14.6 개선사항(`status` 필드, `thoughts_token_count` 직접 캡처, §14.6.1의 재실행 백업)을 반영한 뒤, 실제로 재실행해 패치가 작동하는지 검증했다. **이 실행도 `MODEL_NAME`이 여전히 `gemini-flash-lite-latest`로 설정된 상태였다** — 즉 Run A(flash)가 아니라 Run B와 같은 모델 등급이다.

> **재현성에 대한 정직한 기록**: B097은 이 검증 과정에서 **세 차례 실행**됐고, 매번 다른 후보 조합이 나왔다(1건 → 2건 → 3건). `temperature=0.1`은 0이 아니므로 이는 버그가 아니라 예상된 동작이다. 아래는 **최종 실행 결과**이며, 앞선 두 실행분은 §14.6.1 백업 패치 덕분에 `B097.prev-<timestamp>.json` 형태로 로컬에 보존되어 있다. **이 배치의 재현성 자체가 하나의 관측 결과다** — 같은 코드, 같은 프롬프트, 같은 모델에서 3번 실행해 3번 다른 후보가 나왔고, 최종 CONFIRMED는 매번 0건이었다.
>
> **§14.6.1 백업 패치는 이 세 번째 실행에서 실제로 작동이 확인됐다**: 콘솔에 `[INFO] Existing B097.json backed up to B097.prev-20260811T011558.json`이 출력됐고, 같은 세션에서 B083·B011 재실행 시에도 동일하게 확인됐다.

| Batch | status | 검출 | 비고 |
|---|---|---:|---|
| B097 | `CANDIDATES_FOUND` | **3건 (CONFIRMED 0건, 전부 REJECTED_BY_GROUNDING_CHECK)** | 아래 참조 |
| B083 | `CLEAN` | 0건 | **사람이 확정한 실제 CWE-476(§8.2)을 재실행마다 재현 못함** |
| B011 | `CLEAN` | 0건 | 이전에 오판이었던 CWE-416(§8.3)도 재실행마다 아예 안 나옴 |

**B083·B011이 0건인 것은 새로운 발견이 아니라 §8.4의 "flash-lite는 recall이 낮다"는 관측을 재확인한 것이다.** 같은 모델, 같은 프롬프트에서 반복적으로 실제 취약점을 놓친다는 것은, 이 파이프라인을 하위 등급 모델로 프로덕션에 쓸 경우의 리스크가 일회성이 아니라 **구조적**이라는 근거를 하나 더한다.

**B097의 후보 3건은 원본 코드와 대조한 결과 전부 같은 유형의 오류다 — 함수는 실존하지만 주장된 줄 번호가 크게 틀렸다.**

```json
[
  {
    "cwe": "CWE-125", "function": "mp4_read_box_trak", "line": 409,
    "llm_verifier_status": "REJECTED",
    "verifier_status": "REJECTED_BY_GROUNDING_CHECK"
  },
  {
    "cwe": "CWE-476", "function": "mp4_read_box_mdhd", "line": 472,
    "llm_verifier_status": "CONFIRMED",
    "verifier_status": "REJECTED_BY_GROUNDING_CHECK"
  },
  {
    "cwe": "CWE-787", "function": "mp4_cache_table", "line": 641,
    "llm_verifier_status": "CONFIRMED",
    "verifier_status": "REJECTED_BY_GROUNDING_CHECK"
  }
]
...
원본 저장소(`raspberrypi/userland`, `master`)를 직접 대조하면:

| 주장 | 실제 위치 | 오차 |
|---|---|---|
| `mp4_read_box_trak`, line 409 | **495행**(함수 정의) | **86줄** |
| `mp4_read_box_mdhd`, line 472 | **577행**(함수 정의) | **105줄** |
| `mp4_cache_table`, line 641 | **823행**(함수 정의) | **182줄** |

세 함수 모두 파일 상단(158·161행 전방 선언, 199·202행 박스 타입 디스패치 테이블)에 이름이 등장한다 — LLM이 이 선언·테이블 부분에서 함수 이름은 정확히 읽었지만, **실제 함수 본문의 위치는 매번 다른 엉뚱한 곳에서 가져왔다**고 추정할 수 있다.

> **세 번째 finding(`mp4_cache_table`, CWE-787)은 §9에서 사람이 이미 수동 분석한 바로 그 함수다.** 사람은 `mp4_read_box_stsz` → `mp4_cache_table` → `mp4_read_box_trak`로 3단계 함수를 추적해, `available_entries = size / entry_size`로 `entries`가 먼저 클램핑된 뒤에야 `entries_size = entries * entry_size`를 계산하므로 정수 오버플로우 경로가 막혀 있다고 결론 냈다(REJECTED). 이번 자동 실행이 낸 CWE-787 finding도 **같은 코드를 겨냥했지만**, Grounding Check는 이 안전장치를 이해해서가 아니라 **line 641이 실제 위치(823행)에서 182줄 벗어났다는 이유로** 기각했다. 사람과 자동 파이프라인이 우연히 같은 지점을 조사했지만, 하나는 로직을 이해해서 안전하다고 결론 냈고 다른 하나는 애초에 그 로직 근처에도 가지 못한 채 걸러졌다 — **같은 결론(REJECTED)에 도달한 이유가 근본적으로 다르다는 것**을 보여주는 사례다.

**이 배치에서 가장 주목할 지점은 두 번째·세 번째 finding이다.** 둘 다 LLM Verifier가 `CONFIRMED`로 통과시켰다(`llm_verifier_status: "CONFIRMED"`) — 즉 확률적 에이전트 두 겹(Analyzer, Verifier)이 합의한 판정이었다. 그런데도 **결정론적 Grounding Check가 3건 중 2건에서 이를 뒤집었다.** 이는 §12.2에서 주장한 "LLM 두 겹이 합의해도 결정론 층이 최종 방어선"이라는 설계 원칙이 우연이 아니라 **재현 가능한 동작**임을 보여주는 사례다 — 이번 한 배치 안에서만 2건이 나왔다.

| 환각 유형 | 사례 | 함수/로직 | 줄 번호 | v2 Grounding Check |
|---|---|---|---|---|
| 구문적 (§10.1~10.2) | B083 sprintf/strcpy | 존재하지 않음 | 무의미 | **잡음** |
| 의미적 (§10.3) | B011 CWE-416 | 존재함(진짜) | 정확함 | **못 잡음** |
| **줄 번호 오귀속 (B097, 3건 재현)** | CWE-125(86줄) · CWE-476(105줄) · CWE-787(182줄) | **존재함(진짜)** | **최대 182줄 오차** | **잡음** (±20줄 창 밖, 3/3) |

이 세 번째 유형은 v2 설계의 원래 목적(존재성 검증)이 정확히 겨냥한 실패 모드이고, **B097 한 배치에서만 3건 모두 잡아냈다.** 첫 번째 finding은 LLM Verifier 자신도 이미 거부했었지만(`llm_verifier_status: REJECTED`), Grounding Check가 별도로 대조해 같은 결론(REJECTED_BY_GROUNDING_CHECK)에 도달했다 — 판정 근거는 다르지만(Verifier는 내용을, Grounding Check는 좌표를 봤다) 결과가 일치한 사례다.

**패치 검증 결과 (배치별 실측, 최종 실행 기준)**:

| Batch | Analyzer total | Analyzer thoughts | Verifier total | Verifier thoughts | invisible_tokens_inferred |
|---|---:|---:|---:|---:|---:|
| B083 | 30,124 | `null` | 0 | 0 | 0 (양쪽 다) |
| B011 | 21,127 | `null` | 0 | 0 | 0 (양쪽 다) |
| B097 | 26,770 | `null` | 27,289 | 0 | 0 (양쪽 다) |

`invisible_tokens_inferred`(= total − input − output)는 **실제로 수행된 4개 API 호출**(B083 Analyzer, B011 Analyzer, B097 Analyzer, B097 Verifier)에서 전부 0으로 확인됐다 — §6.2에서 문제였던 "3,928개의 정체불명 토큰이 예산을 잠식"하는 현상이 patched 파이프라인에서는 재발하지 않았다는 실측 근거다.

> **정정**: B083·B011은 Analyzer 결과가 `raw_findings: []`였으므로 `if findings:` 분기에 걸려 **Verifier가 아예 호출되지 않았다**(§4.4의 조건부 호출 설계, §6.3). 위 §8.5 표에 있는 B083·B011의 verifier `total_tokens: 0`은 API가 반환한 값이 아니라, 호출 자체가 없어 초기화된 상태로 남은 **placeholder**다. 따라서 3개 배치라고 해서 API 호출이 6번(배치당 2번) 있었던 것이 아니라, 실제로는 **4번**뿐이었다.

`thoughts_token_count`는 실제로 수행된 Verifier 호출(B097)에서는 API가 명시적으로 `0`을 반환했고, 실제로 수행된 Analyzer 호출 3건에서는 필드 자체가 비어 있었다(`null`) — 이 모델·API 버전 조합에서는 해당 필드를 항상 채워주지 않는다는 뜻으로, §14.5에 남겨둔 "직접 캡처" 개선 자체는 코드 레벨에서 정상 작동하지만 **API가 값을 안 줄 때는 `null`로 정직하게 남는다**는 것까지 확인됐다. 이로써 §11.4·§14.5의 "향후 과제"였던 항목은 **검증 완료**로 격상한다.

`status` 필드도 실행 로그에 정확히 반영됐다: `[CANDIDATES_FOUND]`, `[CLEAN]` — §14.6에서 지적된 "분석 실패와 0건 취약점이 구분되지 않는다"는 문제의 대응책이 실제로 콘솔·JSON 양쪽에 노출됨을 확인했다.

> **`status_counts` 읽는 법에 대한 주석**: `_summary.json`의 `"CANDIDATES_FOUND": 1`은 **후보가 1건**이라는 뜻이 아니라 **후보가 나온 배치가 1개**(B097)라는 뜻이다. 배치 단위 카운터와 후보 단위 카운터를 혼동하지 않도록 이 점을 명시한다. B097 배치 내부의 실제 후보 수는 3건이다.
...
동일 배치(B011)에서의 관측 대조 (§6.3의 한계 주석 참조):
...
**조치 후**: 호출당 3,928개의 비가시 토큰 소비 제거(추정) + 실패 배치 전량 재실행 비용 제거.

> **후속 검증 완료 (§8.5 Run C)**: 위 문단에서 "정확한 수치가 필요하다면 `thoughts_token_count` 필드를 직접 캡처해야 하며, 이는 현재 미구현 상태다"라고 적었던 항목을 실제로 구현하고 재실행했다. 그 결과 3개 배치 전부에서 `invisible_tokens_inferred`(= total − input − output)가 **0**으로 확인됐다 — 조치 이후 재발하지 않았다는 것이 추정이 아니라 실측으로 확인된 것이다. `thoughts_token_count` 자체는 Verifier 호출에서 API가 명시적으로 `0`을 반환했고, Analyzer 호출에서는 필드가 비어 있었다(`null`) — 이 모델·API 버전 조합이 해당 필드를 항상 채워주지는 않는다는 뜻이며, 이 사실 자체도 이제는 추정이 아니라 관측이다. 자세한 실행 로그는 §8.5 참조.
...
**Run A 대비 관측**: B011 기준 42,875 → 21,127 (50.7% 절감)이지만 **후보 1건 → 0건**. 다만 그 1건은 이후 오탐으로 기각됐으므로(§8.3), 이 배치만으로는 recall 저하를 주장할 수 없다 — 유효한 근거는 B083의 확정 CWE-476을 놓친 사례다(§8.4).
...
**입증과 그 한계**: Cppcheck이 0건을 낸 5개 파일에서 LLM이 **검토 후보를 생성**했고(§8.1), LLM이 CONFIRMED 처리한 구문적 환각 2건을 Grounding Check이 기각했다(§10.5). 의미적 환각 1건(§10.3)은 Grounding Check도 통과시켰고 사람이 잡아냈다(§10.7). **§8.5의 재실행에서는 Grounding Check가 LLM Verifier의 `CONFIRMED` 판정 자체를 뒤집은 사례가 한 배치(B097) 안에서만 2건 나왔다**(`mp4_read_box_mdhd`, `mp4_cache_table`) — 두 확률적 에이전트(Analyzer, Verifier)가 합의해도 결정론 층이 최종 방어선이라는 설계가 우연이 아니라 재현 가능함을 보여주는 사례다.

> **다만 "사각지대를 메웠다"고까지 말하지는 않는다.** 이번 3개 배치에서 **Cppcheck이 놓친 취약점을 LLM이 추가로 찾아내 최종 정탐으로 살아남은 사례는 없다.** 최종 CONFIRMED 1건(B083 CWE-476)은 Cppcheck도 이미 찾은 것이다. 정확한 진술은 이것이다 — **LLM 층은 후보 생성 범위를 넓혔고, 기존 진단의 도달 조건·맥락 분석과 triage를 보완했으며, 결정론 층은 그 후보들의 사실 오류를 걸러냈다.** 이것은 상호 보완이지만, "룰 엔진이 못 찾는 걸 AI가 찾아준다"는 주장과는 다르다.

(A)에는 LLM 층이 없고, (B)에는 결정론 층이 없다.

### 12.3 차별점 2 — 불확실성을 명시적 상태로 출력하는 구조
...
- **제한된 예산에서도 사전에 정의한 고위험 공격 표면을 우선적으로 커버할 수 있다**

이는 SLA 협의 시 유용하다: "예산 X에서 Tier N까지 분석합니다"처럼 **분석 범위를 명시적으로 합의**할 수 있다. 다만 Tier 우선순위 자체는 §5.3의 휴리스틱(공격 표면 특성)에 기반한 것이지, Tier 1이 실제 취약점 밀도에서 항상 더 위험함을 실험적으로 증명한 것은 아니다.
...
- 실제로 Run B에서 모델 등급을 낮추자 후보가 3건 → 0건으로 떨어지는 것을 **즉시 감지**했다. 특히 **B083의 확정 CWE-476을 놓쳤다는 사실**을 정답지 덕분에 바로 알 수 있었다(§8.4)
...
- **Run C(§8.5)에서 세 번째로 독립 재현**: 코드 패치 후 재실행했을 때도 B083의 실제 CWE-476(정답지에 CONFIRMED로 등재됨)을 flash-lite 모델이 다시 놓쳤다. 세 번(Run A→B, 그리고 이번 Run C)에 걸쳐 같은 패턴이 반복된다는 것은, 이것이 우연한 결과가 아니라 **모델 등급에 따른 구조적 recall 차이**라는 근거를 강화한다. 정답지가 없었다면 이번에도 "0건 = 안전"으로 오독했을 것이다.
...
| 판정 세분화 | ✕ (이진) | ✕ (모호) | ○ (Verifier 5단계 + Grounding Check override 1종 + 플래그) |
| 재현성 | ○ | ✕ | △ (결정론 층만 보장, 명시) |
...
|---|---|---|---|---|
| **1** | Context Window & 대용량 소스 제약 | Chunker(115배치, 2,500줄 상한) + Tier 우선순위 + C 분석 대상 외 파일 배제 + 조건부 Verifier + thinking 최소화 | 배치당 평균 53,605 토큰으로 118,474줄 저장소 처리 가능. 조건부 호출 50.7% 절감, 비가시 토큰(thinking 추정) 3,928개/호출 제거 | **상당 부분 해결** |
...
| **5** | 사내 자체 프레임워크 학습 부재 | 코드를 **컨텍스트로 직접 제공**하여 사전 학습 불필요. 주석 보존으로 개발자 의도 전달 | **수동 검증 과정**에서 독자 추상화와 Coverity 억제 주석(`lock_order`)까지 컨텍스트로 활용해 CWE-362 후보를 도출했으나, **자동 LLM 파이프라인은 이를 재현하지 못했다**(§9) | **부분 완화** |
...
이 대비는 한계 #5에 대해 **컨텍스트 기반 후보 생성이 갖는 유연성**을 보여준다. 룰 엔진은 모르는 매크로에 대한 규칙이 필요하지만, LLM은 주어진 코드를 읽고 추론한다. 다만 이것이 곧 탐지 정확도의 우위를 뜻하지는 않는다 — 같은 유연성이 §10의 환각 3건을 만들어낸 원인이기도 하다.
...
> **패치 완료 (post-review)**: 이와 별개로, 기존 `run_cppcheck()`는 `subprocess.run()`의 `returncode`를 확인하지 않고 `stderr`를 그대로 Analyzer 힌트로 전달했다. Cppcheck이 인자 오류·크래시 등으로 **비정상 종료해도 그 사실이 결과에 남지 않는** 구조였다. `returncode`를 확인해 `cppcheck_status`(`OK` / `CPPCHECK_NONZERO_EXIT` / `CPPCHECK_NOT_FOUND` / `CPPCHECK_TIMEOUT`)를 결과 JSON에 기록하도록 수정했다. §14.6의 "분석 실패와 0건을 구분한다"는 원칙을 Rule Engine 단계에도 동일하게 적용한 것이다.

### 14.5 Grounding Check 정밀도 — 존재성 검증에서 의미 검증으로

§10.7 참조. v2 Grounding Check는 구문적 환각 2건(§10.1~10.2)과, 이후 재실행에서 관측된 **줄 번호 오귀속 사례 3건**(§8.5, 함수는 실제인데 줄 번호가 86·105·182줄 틀린 경우, 그중 2건은 LLM Verifier가 CONFIRMED로 통과시킨 것까지 잡아냄)은 정확히 잡아냈다 — v2가 원래 겨냥한 실패 모드(존재성 오류)에는 실측으로 작동이 확인됐다. 다만 **함수·라인은 실존하되 결론이 틀린 의미적 환각 1건(§10.3, B011 CWE-416)은 통과시켰다.** 이 사례가 실제로 발생한 뒤에야 그 정확한 실패 모양을 알 수 있었고, 그에 맞춰 v3(Evidence Contract) 설계를 §10.7에 구체화했다. 아직 미구현인 항목:
...
### 14.6 파이프라인 신뢰성 — "분석 실패"와 "취약점 0건"이 코드상 구분되지 않음 (패치 완료·검증됨)

**현황(패치 전)**: 기존 `agent_pipeline.py`의 `process_batch()`는 Analyzer 응답이 JSON으로 파싱되지 않으면 다음과 같이 처리했다.
...
이 경로를 타면 결과 JSON의 `raw_findings`는 `[]`가 되고, **이는 "취약점을 찾아봤는데 없었다"는 정상 결과와 구조적으로 구분되지 않았다.** 실제로 §6.2에서 다룬 초기 thinking-예산 문제 때는 콘솔에 `[WARN]` 로그가 찍혔지만, 그 로그를 보지 않고 저장된 JSON 파일만 봤다면 "이 배치는 깨끗하다"로 오독했을 것이다. 납품용 도구라면 이 구분은 안전(safety)과 직결된다 — 스캔 실패를 스캔 통과로 착각하게 만들 수 있기 때문이다.

**패치 및 검증**: 배치 결과에 `status` 필드를 추가해 `CLEAN`(정상 실행, 후보 0건) / `CANDIDATES_FOUND`(정상 실행, 후보 N건) / `ANALYZER_PARSE_FAILED`(분석 단계 파싱 실패) / `VERIFIER_PARSE_FAILED`(검증 단계 파싱 실패)를 명시적으로 구분하도록 수정했고, `_summary.json`에도 상태별 집계(`status_counts`)를 추가했다. **§8.5(Run C)에서 실제로 재실행해 `[CLEAN]`, `[CANDIDATES_FOUND]`가 콘솔과 JSON 양쪽에 정확히 찍히는 것을 확인했다.** 이 항목은 "개선 방안" 단계에서 "검증 완료" 단계로 마무리됐다.

### 14.6.1 잔여 문제 — 재실행 시 결과 덮어쓰기 (패치 완료·실행 검증됨)

**현황(패치 전)**: `main()`은 배치 결과를 항상 `agent_results/{batch_id}.json`에 썼다. **같은 배치를 다시 실행하면 이전 실행의 산출물이 예고 없이 사라졌다.** 실제로 이것이 §14.7에서 다룬 문제(Run A의 원본이 나중에 실행한 Run B로 덮였던 것)의 근본 원인이었다 — 그때는 `run_A_gemini_flash/` 등으로 **수동 재구성**해서 복구했지만, 코드 자체는 여전히 같은 위험을 안고 있었다. 즉 지금 GitHub에 Run A/B/사람 검증 정답지가 각자 폴더로 보존되어 있는 것은 **사후 수동 조치의 결과**이지, 파이프라인이 재발을 막아주는 것은 아니었다.

**패치**: 배치 출력 경로에 파일이 이미 있으면 덮어쓰기 전에 타임스탬프를 붙여 백업한다.
...
**실행으로 검증됨**: B097·B083·B011 세 배치를 재실행한 결과, 콘솔에 `[INFO] Existing {batch_id}.json backed up to {batch_id}.prev-<timestamp>.json`이 **세 배치 모두에서** 출력됐고(`B097.prev-20260811T011558.json`, `B083.prev-20260811T011637.json`, `B011.prev-20260811T011639.json`), `agent_results/`에 신규 결과와 이전 실행분이 나란히 존재하는 것을 확인했다. **수동 재구성 없이도 실행 이력이 자동 보존된다는 것이 세 배치 모두에서 실측으로 확인됐다.**

> **부수적으로 확인된 사실**: §8.5에 기록된 "CWE-125/CWE-476, 2건" 실행에서는 이 백업이 발생하지 않았었다. 이번 검증을 통해, 그 실행은 **이 패치가 적용되기 전의 구버전 스크립트로 수행됐다**는 가설이 사실로 확인됐다 — 패치 코드 자체에는 결함이 없었다.
...
**대응(적용 완료) — `agent_results/` 디렉터리 재구성**:
...
원본 Run A JSON은 **수정하지 않고 그대로 보존했다**(감사 추적 목적 — LLM이 실제로 뭐라고 말했는지는 그 자체로 기록 가치가 있다). B011 CWE-416의 정정 사유는 `manual_ground_truth/B011.json`에 `_later_correction` 필드로 남겼고, `report.md`도 저장소 루트에 커밋해 README가 가리키는 파일이 실제로 존재하도록 했다. **이 재구성은 적용 완료 상태이며, §15.1의 상태 열에서 확인할 수 있다.**
...
**재현성 자체도 낮다는 것이 §8.5에서 추가로 드러났다**: B097 하나를 세 번 재실행했더니 매번 다른 후보 조합(1건→2건→3건)이 나왔다. `temperature=0.1`인 이상 예상된 동작이지만, 이는 "3개 배치의 결과"라는 것 자체가 **그 순간의 스냅샷**이지 그 배치의 고정된 특성이 아님을 뜻한다. 표본 크기(6.0%)의 한계에 더해, **같은 배치를 다시 돌려도 같은 결과가 나온다는 보장이 없다**는 것까지 감안해야 한다.

**개선 방안**: 쿼터/예산 확보 후 Tier 1~2 전량(약 30~40배치) 실행 + 사람 검증 정답지를 10배치 이상으로 확대. 재현성 문제에 대해서는 동일 배치를 N회 반복 실행해 후보 등장 빈도를 집계하는 방식(예: "이 finding은 5회 중 3회 등장")도 향후 과제로 고려할 수 있다.
...
| `agent_pipeline.py` | Analyzer / Verifier 멀티 에이전트 파이프라인 + Grounding Check | 푸시 완료 (§14.6 패치 반영 버전) |
| `agent_results/run_A_gemini_flash/*.json` | **Run A 실제 결과** — B083 CONFIRMED 3건(2건은 이후 환각으로 판명), B097 REJECTED 2건, B011 CONFIRMED 1건(이후 REJECTED로 정정) | 푸시 완료 (§14.7 재구성 반영) |
| `agent_results/run_B_flash_lite/*.json` | 모델 등급 교체 실험, 0건 | 푸시 완료 |
| `agent_results/manual_ground_truth/*.json` | 사람 검증 정답지 + B011 CWE-416 정정 사유(가드 절 코드 인용 포함) | 푸시 완료 |
| `repo_files.xml` | 저장소 파일 목록 | 푸시 완료 |
| `report.md` | 본 보고서 | 푸시 완료 |
| `.gitignore` | `userland/`, `.env`, `*.key` 등 제외 | 푸시 완료 |
| `SAST_Agent_발표.pptx` | 발표 자료 12장 (구성은 §16.2와 1:1 일치) | 별도 제출물 |

**`userland/` 소스는 저장소에 포함하지 않았다.** 원저작권 존중 및 저장소 비대화 방지 목적이며, 별도 클론해 사용한다.

> **"상태" 열을 둔 이유**: 초고 시점에는 보고서가 인용하는 실험 결과와 실제 저장소 내용 사이에 간극이 있었다(§14.7). 그 간극은 `agent_results/`를 3개 디렉터리로 재구성하고 `report.md`를 커밋하면서 해소됐으며, **현재 위 항목은 전부 저장소에 반영된 상태다.**
...
**슬라이드 수는 두 번 바뀌었다.** 초고는 13장이었고, 10분에 13장이면 한 장당 46초라 어느 것도 제대로 설명할 수 없어 **9장으로 압축**했다. 이후 실제 제작 단계에서 **12장으로 다시 늘렸다** — 다만 늘어난 3장은 내용이 아니라 **구조**다: 표지(1장), 아젠다(2장), 그리고 **프롬프트 엔지니어링 전용 슬라이드(9장)**. 앞의 둘은 발화 시간을 거의 쓰지 않는 진입/네비게이션 장이고, 프롬프트 장은 **평가 기준 ③에 정면 대응하는 내용이 9장 구성에서 누락되어 있었기 때문에 추가**했다. 반면 CWE Top 25의 역사나 Cppcheck 버전 연혁처럼 보고서에는 근거로 필요해도 발표의 핵심이 아닌 내용은 계속 배제했다.

| 원칙 | 적용 |
|---|---|
| **실패를 먼저, 그것도 가장 세게 판다** | 슬라이드 8이 발표의 클라이맥스 — 세 가지 환각 사례 중 가장 위험한 유형(§10.3)을 정면에 배치 |
...
| **구조 장은 시간을 쓰지 않는다** | 표지·아젠다는 합쳐 40초 이내로 넘긴다 |

### 16.2 슬라이드 구성 (12장, 10분 기준)

실제 제작된 발표 파일(`SAST_Agent_발표.pptx`)의 구성과 1:1로 일치한다.

| # | 슬라이드 | 시간 | 핵심 메시지 | 화면에 띄울 근거 |
|---|---|---|---|---|
| 1 | **표지 — "AI가 SAST를 대체할 수 있는가. 아직, 아니다."** | 0:15 | 결론을 먼저 던지되, 근거는 지금부터 보여준다고 예고 | 실행 콘솔 터미널 (6건 → CONFIRMED 1건, 환각 3건) |
| 2 | **아젠다** | 0:15 | "실패부터 판다. 그다음 설계." 발표 원칙을 명시 | 챕터별 시간 배분표 |
| 3 | **CH01 · 5대 한계** | 0:50 | 진짜 질문은 "코드 넣고 물으면 되나"가 아니라 "이 5개 한계를 아키텍처로 얼마나 상쇄하는가" | AI SAST 5대 한계 카드 (#2 강조) |
| 4 | **CH02 · Target userland** | 0:50 | 아카이브된 레거시 C 118,474줄. 스코프 10종 중 6종이 2025 Top 25 안에 있다 | pygount 실측 + MITRE 순위·KEV 표 |
| 5 | **CH03 · 아키텍처** | 1:00 | **"LLM은 샌드위치의 속이지 빵이 아니다"** — 이 발표의 한 줄 | 4단계 파이프라인 (결정론/LLM 이중 언어) |
| 6 | **CH04 · Chunking + 토큰 절감** | 0:50 | 배치 우선순위는 코드 양이 아니라 **신뢰 경계** | Tier 규칙 + 50.7% 실측 + 5계층 |
| 7 | **CH05 · 3개 배치 결과** | 0:55 | 자동 파이프라인이 낸 6건 → **최종 CONFIRMED 1건**. 서로 다른 공격 표면을 대표하는 3개 배치를 실행 전 선정했다 | 배치별 판정 카드 3장 |
| 8 | **CH06 · ⚡ 환각 3건** | **1:30** | **클라이맥스.** 구문적 환각 2건은 코드가 잡았지만, "함수·라인 다 맞는데 결론이 틀린" 의미적 환각은 Grounding Check도 통과시켰고 사람이 원본 재대조로만 잡아냈다 | 환각 3유형 코드 대조 (모노스페이스) |
| 9 | **CH07 · 프롬프트 엔지니어링** | 1:00 | "잘 검증해줘"는 무시된다. 그래서 생각을 **필수 필드로 강제**했다 | (B) 개발 중 실제 입력 프롬프트 + (A) 도구 내부 프롬프트 |
| 10 | **CH08 · v2 → v3 Evidence Contract** | 0:50 | *"검증하라"는 지시로는 검증을 강제할 수 없다. 진짜 대조는 코드로 한다* | v2 체크 3종 + v3 스키마 |
| 11 | **CH09 · 3자의 상호 사각지대** | 0:50 | 셋은 서로 다른 곳을 본다. 이 표본에서 자동 파이프라인이 먼저 찾은 최종 정탐은 없다 | §9 3행 비교표 |
| 12 | **CH10 · 결론 + 5대 한계 대응** | 0:55 | **"이번 실험에서 AI가 더 많은 최종 정탐을 확보했다는 증거는 없다"** — 가치는 대체가 아니라 분석·triage·검증 보조 | 5대 한계 대응 매핑표 (해결 수준 점 스케일) |

**합계 정확히 10분 00초** (0:15+0:15+0:50+0:50+1:00+0:50+0:55+1:30+1:00+0:50+0:50+0:55 = 600초). 표지·아젠다는 합 30초로 빠르게 넘긴다. 슬라이드 8에 가장 많은 시간을 배정한 이유는, 이 프로젝트의 진짜 기여가 "취약점을 찾았다"가 아니라 **"검증 계층 자체의 한계를 실측하고 다음 설계로 연결했다"**는 데 있기 때문이다.
...
| "왜 에이전트가 2개뿐인가?" | 에이전트를 늘리면 호출 수만큼 토큰과 오류 표면이 선형 증가합니다. **서로 다른 실패 모드를 잡는 역할만 분리**했습니다. Dedup·Ranking은 결정론적 코드가 적합한 영역이라 LLM 에이전트로 만들지 않았고, 이번 표본에서는 중복이 없어 아직 구현하지 않은 상태입니다(§3.3, §14.8) |

---

## 17. 부록

### 17.1 도구 내부 프롬프트 전문 (A) — Analyzer / Verifier 시스템 프롬프트

> 아래는 `agent_pipeline.py`에 하드코딩된 **실사용 v1 프롬프트 원문**이다. 보고서 검증을 위해 수정 없이 보존했다. **개발 과정에서 개발자가 AI 어시스턴트에게 직접 입력한 프롬프트(B)는 §7.2에 별도로 수록했다.**
...
#### API 호출 파라미터 (v2.1 — patched pipeline, §14.6 반영)
...
# v2.1 개선(§14.5/§14.6): thoughts_token_count를 직접 캡처하고,
# 분석 실패(status)를 0건(findings=[])과 명확히 구분한다.
thoughts_tok = getattr(usage, "thoughts_token_count", None) if usage else None
status = "CANDIDATES_FOUND"  # | "CLEAN" | "ANALYZER_PARSE_FAILED" | "VERIFIER_PARSE_FAILED"
```
...
>
> **마지막 검증 단계(Run C, §8.5)**: §14.5·§14.6의 코드 패치(상태 필드, `thoughts_token_count` 캡처)를 실제로 적용하고 재실행해, "향후 과제"로 남겨뒀던 항목들이 실제로 작동하는지 확인했다. 그 과정에서 세 번째 환각 유형(줄 번호 오귀속)과 recall 문제의 3회차 재현이라는 두 가지 추가 데이터를 얻었다. 이 보고서가 정정을 여러 차례 거친 것은 결함이 아니라, §10.5~10.7에서 주장한 원칙("AI의 출력은 코드로 검증하고, 그 검증 결과도 다시 실행해서 확인한다")을 이 문서 자신에게도 그대로 적용한 결과다.
...
- Cppcheck이 0건을 낸 5개 파일에서 LLM이 **검토 후보를 생성**했다 (§8.1, 단 이 실험의 minimal-config baseline 기준이며, 그중 최종 정탐으로 살아남은 것은 없다)
...
> **이번 실험에서 AI가 기존 SAST보다 더 많은 최종 정탐을 확보했다는 증거는 얻지 못했다.** 사람이 재검증한 최종 결과에서 CONFIRMED는 B083의 CWE-476 1건뿐이고, 그것은 Cppcheck도 이미 찾은 것이다. **대신 확인된 것은 보완 가치다** — 기존 진단의 도달 조건과 맥락을 설명하고, 룰 엔진이 보지 않은 영역에서 추가 검토 후보를 생성하며, `runtime_dependent`·5단계 판정처럼 불확실성을 구조화한다. **따라서 현재 단계에서 AI의 가치는 SAST 대체가 아니라 분석·triage·검증 보조에 있다.**
>
> 그리고 그 보조 가치를 실제로 쓰려면, AI의 출력을 다시 결정론적으로 검증하는 층이 필요하다 — 다만 그 검증 층도 "존재하는가"만 확인해서는 부족하고, "결론이 타당한가"까지 확인해야 한다는 것을 이 프로젝트 스스로 증명했다.
...
*작성: 이유진 | 초안 2026-08-10, 리뷰 반영 정정 2026-08-10*