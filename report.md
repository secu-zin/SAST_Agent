...
`invisible_tokens_inferred`(= total − input − output)가 **3개 배치, 6개 API 호출 전부에서 0으로 확인**됐다 — §6.2에서 문제였던 "3,928개의 정체불명 토큰이 예산을 잠식"하는 현상이 patched 파이프라인에서는 재발하지 않았다는 실측 근거다. `thoughts_token_count`는 Verifier 호출에서는 API가 명시적으로 `0`을 반환했고, Analyzer 호출에서는 필드 자체가 비어 있었다(`null`) — 이 모델·API 버전 조합에서는 해당 필드를 항상 채워주지 않는다는 뜻으로, §14.5에 남겨둔 "직접 캡처" 개선 자체는 코드 레벨에서 정상 작동하지만 **API가 값을 안 줄 때는 `null`로 정직하게 남는다**는 것까지 확인됐다. 이로써 §11.4·§14.5의 "향후 과제"였던 항목은 **검증 완료**로 격상한다.
...
| 7 | **CH05 · 3개 배치 결과** | 0:55 | 자동 파이프라인이 낸 6건 → **최종 CONFIRMED 1건**. "CONFIRMED만 3건"을 의도적으로 피했다 | 배치별 판정 카드 3장 |
...
> **이번 실험에서 AI가 기존 SAST보다 더 많은 최종 정탐을 확보했다는 증거는 얻지 못했다.** 사람이 재검증한 최종 결과에서 CONFIRMED는 B083의 CWE-476 1건뿐이고, 그것은 Cppcheck도 이미 찾은 것이다. **대신 확인된 것은 보완 가치다** — 기존 진단의 도달 조건과 맥락을 설명하고, 룰 엔진이 보지 않은 영역에서 추가 검토 후보를 생성하며, `runtime_dependent`·6단계 판정처럼 불확실성을 구조화한다. **따라서 현재 단계에서 AI의 가치는 SAST 대체가 아니라 분석·triage·검증 보조에 있다.**
>
