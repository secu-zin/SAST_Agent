...
`agent_pipeline.py`에 §14.5~14.6 개선사항(`status` 필드, `thoughts_token_count` 직접 캡처)을 반영한 뒤, 실제로 재실행해 패치가 작동하는지 검증했다. **이 실행도 `MODEL_NAME`이 여전히 `gemini-flash-lite-latest`로 설정된 상태였다** — 즉 Run A(flash)가 아니라 Run B와 같은 모델 등급이다.

| Batch | status | 검출 | 비고 |
|---|---|---:|---|
| B097 | `CANDIDATES_FOUND` | 1건 (CONFIRMED 0건) | 아래 참조 — 새로운 유형의 오류 |
| B083 | `CLEAN` | 0건 | **사람이 확정한 실제 CWE-476(§8.2)을 이번에도 재현 못함** |
| B011 | `CLEAN` | 0건 | 이전에 오판이었던 CWE-416(§8.3)도 이번엔 아예 안 나옴 |

**B083·B011이 0건인 것은 새로운 발견이 아니라 §8.4의 "flash-lite는 recall이 낮다"는 관측을 세 번째로 재확인한 것이다.** 같은 모델, 같은 프롬프트에서 반복적으로 실제 취약점을 놓친다는 것은, 이 파이프라인을 하위 등급 모델로 프로덕션에 쓸 경우의 리스크가 일회성이 아니라 **구조적**이라는 근거를 하나 더한다.

**B097에서 나온 finding은 지금까지의 두 환각 유형과 다른 세 번째 패턴이다**:

```json
{
  "cwe": "CWE-120",
  "file": "containers/mp4/mp4_reader.c",
  "function": "mp4_read_box_trak",
  "line": 409,
  "evidence": "if(p_ctx->tracks_num >= MP4_TRACKS_MAX) return ...; 이 경계 검사 로직을 근거로 들었으나, tracks 배열 조작 시 경계가 깨질 수 있다고 우려"
}
```

원본 코드를 직접 대조한 결과: `mp4_read_box_trak` 함수와 그 경계 검사 로직은 **전부 실제로 존재한다** — 다만 line 409가 아니라 **495~522행**에 있다. 함수명·로직·인용한 코드까지 다 진짜인데 **줄 번호만 86줄 틀렸다.**
...
| **줄 번호 오귀속 (신규, 이 실행)** | B097 CWE-120 | **존재함(진짜)** | **86줄 오차** | **잡음** (±20줄 창 밖) |

이 세 번째 유형은 v2 설계의 원래 목적(존재성 검증)이 정확히 겨냥한 실패 모드이고, 실제로 잡아냈다. **다만 LLM Verifier 스스로도 이미 이 finding을 낮은 확신(confidence 0.4)으로 취급했다** — Grounding Check가 통과시켰어도 최종 채택 전에 걸러졌을 가능성이 높다는 뜻으로, 두 검증 층이 서로 다른 각도에서 같은 결론에 도달한 사례로 볼 수 있다.

**패치 검증 결과**:

```json
"token_usage": {
  "analyzer":  {"total_tokens": 30124, "thoughts_token_count": null, "invisible_tokens_inferred": 0},
  "verifier":  {"total_tokens": 0,     "thoughts_token_count": 0,    "invisible_tokens_inferred": 0}
}
```

`invisible_tokens_inferred`(= total − input − output)가 3개 배치 전부에서 **0으로 확인**됐다 — §6.2에서 문제였던 "3,928개의 정체불명 토큰이 예산을 잠식"하는 현상이 patched 파이프라인에서는 재발하지 않았다는 실측 근거다. `thoughts_token_count`는 Verifier 호출에서는 API가 명시적으로 `0`을 반환했고, Analyzer 호출에서는 필드 자체가 비어 있었다(`null`) — 이 모델·API 버전 조합에서는 해당 필드를 항상 채워주지 않는다는 뜻으로, §14.5에 남겨둔 "직접 캡처" 개선 자체는 코드 레벨에서 정상 작동하지만 **API가 값을 안 줄 때는 `null`로 정직하게 남는다**는 것까지 확인됐다. 이로써 §11.4·§14.5의 "향후 과제"였던 항목은 **검증 완료**로 격상한다.
...
**입증과 그 한계**: Cppcheck이 0건을 낸 5개 파일에서 LLM이 **검토 후보를 생성**했고(§8.1), LLM이 CONFIRMED 처리한 구문적 환각 2건을 Grounding Check이 기각했다(§10.5). 의미적 환각 1건(§10.3)은 Grounding Check도 통과시켰고 사람이 잡아냈다(§10.7).
...
§10.7 참조. v2 Grounding Check는 구문적 환각 2건(§10.1~10.2)과, 이후 재실행에서 관측된 **줄 번호 오귀속 사례**(§8.5, 함수·로직은 실제인데 줄 번호만 86줄 틀린 경우)는 정확히 잡아냈다 — v2가 원래 겨냥한 실패 모드(존재성 오류)에는 실측으로 작동이 확인됐다. 다만 **함수·라인은 실존하되 결론이 틀린 의미적 환각 1건(§10.3, B011 CWE-416)은 통과시켰다.** 이 사례가 실제로 발생한 뒤에야 그 정확한 실패 모양을 알 수 있었고, 그에 맞춰 v3(Evidence Contract) 설계를 §10.7에 구체화했다. 아직 미구현인 항목:
...
### 14.6.1 잔여 문제 — 재실행 시 결과 덮어쓰기 (패치 완료)
...
이제 같은 배치를 다시 돌려도 이전 실행 결과가 `B083.prev-20260810T174500.json` 형태로 남아, **수동 재구성 없이도 실행 이력이 자동 보존**된다.

> **상태명을 `FINDINGS_FOUND`가 아니라 `CANDIDATES_FOUND`로 정한 이유**: 이 단계의 산출물은 **검증 전 후보**다. 실제로 Run C의 B097은 `CANDIDATES_FOUND`(1건)이지만 최종 CONFIRMED는 0건이었다(§8.5). 납품 문맥에서 "finding"은 확정된 이슈로 읽히기 쉬우므로, 상태명 자체가 확정성을 암시하지 않도록 바꿨다. 용어 하나가 오독을 만드는 것도 §14.6이 다루는 신뢰성 문제의 일부다.
