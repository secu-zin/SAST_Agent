...
| Batch | status | 검출 | 비고 |
|---|---|---:|---|
| B097 | `FINDINGS_FOUND` | 1건 (CONFIRMED 0건) | 아래 참조 — 새로운 유형의 오류 |
...
`status` 필드도 실행 로그에 정확히 반영됐다: `[FINDINGS_FOUND]`, `[CLEAN]` — §14.6에서 지적된 "분석 실패와 0건 취약점이 구분되지 않는다"는 문제의 대응책이 실제로 콘솔·JSON 양쪽에 노출됨을 확인했다.
...
**패치 및 검증**: 배치 결과에 `status` 필드를 추가해 `CLEAN`(정상 실행, 0건) / `FINDINGS_FOUND`(정상 실행, N건) / `ANALYZER_PARSE_FAILED`(분석 단계 파싱 실패) / `VERIFIER_PARSE_FAILED`(검증 단계 파싱 실패)를 명시적으로 구분하도록 수정했고, `_summary.json`에도 상태별 집계(`status_counts`)를 추가했다. **§8.5(Run C)에서 실제로 재실행해 `[CLEAN]`, `[FINDINGS_FOUND]`가 콘솔과 JSON 양쪽에 정확히 찍히는 것을 확인했다.** 이 항목은 "개선 방안" 단계에서 "검증 완료" 단계로 마무리됐다.
